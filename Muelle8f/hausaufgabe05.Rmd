...
dat <- read.table("Muelle8f//CopyOfbody_dim_long.tab",header=TRUE) 
...
# Überlegung: Es besteht immer die Gefahr, die Ergebnisse zu verfälschen, wenn man bestimmte Daten (einfach) weglässt. Dieser Schritt sollte deshalb sehr gut überlegt und begründet sein (v.a. auch in Bezug auf die Fragestellung). An dieser Stelle dürfte man dann zum Beispiel nicht danach fragen, ob es einen Unterschied in der Größe Studierender der jeweiligen Studiengänge gibt, sondern, ob es einen Unterschied in der Größe der Frauen gibt, beipsielweise: "Sind weibliche Studierende der 'Klinischen Linguistik' größer als solche der anderen untersuchten Studiengänge?" 
...
# Antwort: Nein, auf den ersten Blick wirken die Darstellungen unterschiedlich. 

# Welche Gruppe hat gefehlt? Wie viele Datenpunkte gab es für die Gruppe?
# (Keine explizite Antwort nötig, nur eine Überlegung.)

# Antwort: Die Studiengänge "Germanistische Linguistik" (eine Person), "Other" (zwei Personen) fehlen in der Darstellung der Dichte. Wahrscheinlich ist eine Darstellung der Dichte aufgrund der geringen Anzahl von Studierenden dieser Studiengänge nicht möglich. Hier zeigt sich, dass die Box-Whisker den Vorteil bietet, diese Gruppen trotzdem darstellen zu können.  
...
# M.A. Klinsche Linguistik - Standardabweichung für die Größe 
x <- (klinisch$height)
klinisch.abweichung <- x - mean(x)
...
# M.A. Linguistik Kognition und Kommunikation - Standardabweichung für die Größe
y <- (linkk$height)
linkk.abweichung <- y - mean(y)
linkk.quadr.abweichung <- linkk.abweichung^2
varianz.y <- mean(linkk.quadr.abweichung)
print(varianz.y)

linkk.sd <- sqrt(varianz.y)
print(linkk.sd)

# M.A. Speech Science - Standardabweichung für die Größe 
z <- (speech$height)
speech.abweichung <- z - mean(z)
speech.quadr.abweichung <- speech.abweichung^2
varianz.z <- mean(speech.quadr.abweichung)
print(varianz.z)
...
print( paste("Studiengang: Kognition und Kommunikation","Mean:",mean(linkk$height),"SD:",linkk.sd) )
